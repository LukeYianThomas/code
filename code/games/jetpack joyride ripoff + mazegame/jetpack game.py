...
def collideCheck(z,c):
    ...
    for obstacle_rect in obstacles:
        ...
        if playerHitbox.clipline(right):
            xVel=xVel*-1
    if playerHitbox.clipline((0,screen_height-20,screen_width*10,screen_height-20)):
            yVel=0
...
def obstacle_generation(amount):
    ...
    for _ in range(amount):
        _x,_y=(round(random.randint(180,screen_width*10-size),75),round(random.randint(0,screen_height-size),75))
...
def minimap():
    ...
    for obstacle_rect in obstacles:
        _x,_y,size = obstacle_rect[0]
        pygame.draw.rect(screen, "red",pygame.Rect(479+_x*scaler3,1+_y*scaler3,7.5,7.5))

def backgroundRender():
    pygame.draw.rect(screen,"white",pygame.Rect(0,0,screen_width,screen_height))
    pygame.draw.rect(screen,"gray",pygame.Rect(0,580,screen_width,640))
#things to run before game starts
obstacle_generation(200)
#render game
